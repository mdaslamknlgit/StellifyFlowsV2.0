...
@Component({
  ...
  styleUrls: ["./email-import.component.css"],
})
export class EmailImportComponent implements OnInit {
  ...
  getList(): void {
    const self = this;
    this.listService.GetList(0, this.UserId).subscribe(
      (listResult) => {

        //debugger;
        this.Listss = listResult.Lists;
        this.MyLists = listResult.Lists;

        if (this.Listss.length > 0) {
          //TODO
          this.listsControl.setValue(0, { onlySelf: true });
          this.SelectedList = 0;
          //this.CampaignForm.controls["TemplateId"].setValue(CampaignsInfoRes.TemplateId, {onlySelf: true});
        }
      },
      (err) => {
        console.log(err);
  ...
  uploadDocument() {
    ...
    if (this.SelectedList == 0) 
    {
      if(this.ImportCSVForm.controls["ListName"].value==null)
      {
        this.ImportCSVForm.controls["ListName"].markAsTouched();
        this.renderer.selectRootElement('#ListName').focus();
        return false;
      }
      else{
        this.SelectedListName=this.ImportCSVForm.controls["ListName"].value;
      }
      // if (this.ListNameFormControl.value == "" &&this.OnlySelectedList == false ) 
      // {
      //   this.MyListNameRef.nativeElement.focus();
    ...
    //let SelectedListName = this.ListNameFormControl.value;

    //debugger;
    if(this.SelectedListName=="")
    {
      this.SelectedListName=this.MyListNameRef.nativeElement.value;
    }
    ...
    if (this.IsMapped) 
    {
      ...
      formData.append("ListId", this.SelectedList.toString());
      formData.append("ListName", this.SelectedListName);
      formData.append("FileType", this.FileType);

      let fileUploadRoute = environment.apiEndpoint + "CSVAutomapUpload" + `/${0}/${this.UserId}`;
      //debugger;
      this.listService.UploadFile(formData, this.uploadedFiles, this.UserId, this.CompanyId)
        .subscribe((data:any) => {
          debugger;
          //ErrorMessage
