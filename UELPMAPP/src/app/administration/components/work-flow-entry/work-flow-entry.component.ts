...
@Component({
  selector: 'app-work-flow-entry',
  templateUrl: './work-flow-entry.component.html',
  styleUrls: ['./work-flow-entry.component.css'],
...
export class WorkFlowEntryComponent {
  ...
  workFlowConfigurationsCols:any[];
  ...
  constructor(private fb: FormBuilder, private workFlowApiService: WorkFlowApiService,
    ...
    this.LogCompanyId = this.sessionService.getCompanyId();
    this.initDone = true;
  ...
  ngOnInit() {
    ...
    this.activatedRoute.paramMap.subscribe((param: ParamMap) => {
      ...
      if (param.get('ProcessId') != undefined) {
        this.ProcessId = parseInt(param.get('ProcessId'));
      }
      if (param.get('CompanyId') != undefined) {
        this.CompanyId =parseInt(param.get('CompanyId'));
      }
      if (param.get('LocationId') != undefined) {
        this.LocationId = parseInt(param.get('LocationId'));
      }
    });
    ...
    this.getUsers();
    this.getUserRoles();
    this.getDepartments(this.LogCompanyId);
    
    debugger;
    if(this.FormMode=="EDIT")
    {
      this.getWorkFlowProcesses(this.ProcessId,this.CompanyId,this.LocationId);
      //this.getWorkFlowConfiguration(this.ProcessId,this.CompanyId,this.LocationId)
    }
    else
    {
      this.getWorkFlowProcesses(0,0,0);
      this.addData();
    }
  ...
  getWorkFlowProcesses(processId: number, companyId: number, locationId: number): void {
    let workFlowProcess = {
      searchKey: ""
    };
    ...
    workFlowProcessResult.subscribe((data) => {
      ...
      // else
      // {
      //   this.getWorkFlowConfiguration(this.ProcessId,this.LogCompanyId,this.LocationId);
      // }
  ...
  onDocumentChange(event) {
    ...
    this.mandatoryWorkflow = this.workFlowProcesses.filter(x => x.ProcessId == this.SelectedProcessId)[0].IsMandatoryFollowWorkflow;
    let processName = this.workFlowProcesses.filter(x => x.ProcessId == this.SelectedProcessId)[0].ProcessName;
    ...
    // if (this.ProcessId > 0 && this.defaultLocationId > 0) {
      ...
      else
      {
        ...
        this.getWorkFlowConfiguration(this.SelectedProcessId, this.LogCompanyId, this.defaultLocationId);
      }
  ...
  onDepartmentChange(event) {
    ...
    debugger;
    if (this.ProcessId > 0 && this.defaultLocationId > 0) {
      this.getWorkFlowConfiguration(this.ProcessId, this.LogCompanyId, this.defaultLocationId);
    }
  }
  showLeftCol(event) {
  ...
  EditWorkFlow(configuration: WorkFlowConfigurationList) {
    this.showfilters =false;
  ...
  ClickBack(e)
  {
    ...
    this.router.navigate([`/admin/workflowl`]);
