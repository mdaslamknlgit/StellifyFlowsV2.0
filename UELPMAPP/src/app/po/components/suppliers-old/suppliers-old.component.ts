...
@Component({
  selector: 'app-suppliers-old',
  templateUrl: './suppliers-old.component.html',
  styleUrls: ['./suppliers-old.component.css'],
  providers: [{ provide: NgbDateAdapter, useClass: NgbDateNativeAdapter }]
})
export class SuppliersOldComponent implements OnInit {
  FileName:string="";
  FileNo:number=0;
  CurrentDate:string="";
  exportColumns;
  ...
  getSuppliers(): void {
    ...
    supplierResult.subscribe((data) => {
      ...
      this.FilteredSuppliersList=data.Suppliers;
...
ExportToCSV() {
  ...
  this.supplierPagerConfig.RecordsToSkip = 0;
  this.supplierPagerConfig.RecordsToFetch = 10000;
  ...
  this.interval = setTimeout(() => {
      ...
      if (this.IsFilterDataArrive) {
          debugger;
          this.resetPagerConfig();
          if (this.FilteredSuppliersList.length > 0) {
              debugger;
              this.CurrentDate=moment().format('YYYYMMDD').toLocaleString();
              this.FileNo=Math.ceil(Math.random() * 20);
              this.FileName="SuppliersList_" + this.CurrentDate+"_"+this.FileNo.toString();
              ...
              const options = {
                  ...
                  headers: ['Supplier Code', 'Supplier Name', 'Category', 'Status',,'CreatedOn']
              };
              const csvExporter = new ExportToCsv(options);
              csvExporter.generateCsv(this.FilteredSuppliersList);
              this.stopTime();
          }
          else
          {
...
exportToExcel(): void {
  ...
  const udt: ExcelJson = {
      data: [
          { A: 'Suppliers Lists' }, // title
          { A: 'Supplier Code', B: 'Supplier Name', C: 'Category',D: 'Status',E:'Created On' }, // table header
      ],
      skipHeader: true
  };
  this.FilteredSuppliersList.forEach(contact => {
      udt.data.push({
          A: contact.SupplierCode,
          B: contact.SupplierName,
          C: contact.CategoryName,
          D: contact.WorkFlowStatus,
          E: contact.CreatedDate,
          F: contact.EmailId
      });
  });
  edata.push(udt);

  this.exportService.exportJsonToExcel(edata, 'SuppliersLists');
}
ExportToExcel() {
  const self=this;
  ...
  this.interval = setTimeout(() => {
      ...
      if (this.IsFilterDataArrive) {
          ...
          if (this.FilteredSuppliersList.length > 0) {
              ...
              this.FilteredSuppliersList.forEach(polist => {
                  udt.data.push({
                    A: polist.SupplierCode,
                    B: polist.SupplierName,
                    C: polist.CategoryName,
                    D: polist.WorkFlowStatus,
              ...
              this.FileName="SuppliersLists_" + this.CurrentDate+"_"+this.FileNo.toString();
  ...
  this.interval = setTimeout(() => {
      ...
      if (this.IsFilterDataArrive) {
          ...
          if (this.FilteredSuppliersList.length > 0) {
              ...
              this.exportColumns = this.filteredSuppliersCols.map((col) => ({
                  title: col.header,
                  dataKey: col.field,
              }));
              ...
              doc['autoTable'](this.SelexportColumns, this.FilteredSuppliersList);
